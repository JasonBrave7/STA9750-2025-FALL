...
str(GLOBAL_TOP_10)
```
Here we can see different column names, and then followed by their type. Most of the data here are strings, which are referred to as characters
```{r}
...
glimpse(GLOBAL_TOP_10)
```
Here, we changed the season titles to actually come across as NA.
```{r}
library(readr)
library(dplyr)
library(stringr)
